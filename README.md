...
The three database collections should stay on creator-only permissions. The
API is the only read boundary for the Web panel.
...
panel renders sample data so the layout can be reviewed.
...
- reads the current version of `hn_dashboard_ingest_runs`;
- reads the current version of `hn_dashboard_cloud_sync_runs`;
- enforces `OPS_DASHBOARD_TOKEN`;
- returns one JSON snapshot for the Web UI.
